...
router.get('/cat/:name', async (req, res) => {
    ...
    const books = await models.book.find({"categories.name": catName, "price": {$gte: priceRange[0], $lte: priceRange[1]}} ).sort(sortFilter);
...
router.get('/store/:name', async (req, res) => {
    ...
    const storesStock = (await models.store.find({name:  storeName}, {storeStock: 1})).map(store => store.storeStock);
    ...
    const books = await models.book.find({_id: {$in: bookIds}, "price": {$gte: priceRange[0], $lte: priceRange[1]}}).sort(sortFilter);
